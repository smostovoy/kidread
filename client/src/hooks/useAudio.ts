...
export function useAudio() {
  ...
  const playApplause = useCallback(() => {
    // Try to play custom success audio first
    const customSuccessPath = '/audio/effects/success.mp3';
    
    fetch(customSuccessPath, { method: 'HEAD' })
      .then(response => {
        if (response.ok) {
          playCustomAudio(customSuccessPath);
        } else {
      ...
      .catch(() => {
        ...
        try {
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            if (time < 0.3) {
              sample += Math.sin(2 * Math.PI * 523.25 * time) * Math.exp(-time * 3) * 0.3;
            }
            if (time >= 0.2 && time < 0.5) {
              sample += Math.sin(2 * Math.PI * 659.25 * (time - 0.2)) * Math.exp(-(time - 0.2) * 3) * 0.3;
            }
            if (time >= 0.4 && time < 0.8) {
              sample += Math.sin(2 * Math.PI * 783.99 * (time - 0.4)) * Math.exp(-(time - 0.4) * 3) * 0.3;
            }
